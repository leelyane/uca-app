<x-action-section>
    ...
    <x-slot name="description">
        {{ __('Ajoutez une sécurité supplémentaire à votre compte grâce à Two Factor Authentification.') }}
    </x-slot>
        ...
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                @if ($showingConfirmation)
                    {{ __('Terminer l\'activation de two factor authentication.') }}
                @else
                    {{ __('Vous avez activé two factor authentication.') }}
                @endif
            @else
                {{ __('Vous n\'avez pas activé two factor authentication.') }}
            @endif
        </h3>
            ...
            @if ($showingQrCode)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        ...
                        @else
                            {{ __('Two Factor Authentification est désormais activée. Scannez le code QR suivant à l\'aide de l\'application d\'authentification de votre téléphone ou saisissez la clé de configuration.') }}
                        @endif
                    </p>
                </div>
            ...
            @else
                @if ($showingRecoveryCodes)
                    <x-confirms-password wire:then="regenerateRecoveryCodes">
                        <x-secondary-button class="me-3">
                            {{ __('Regénérer Codes de récupération') }}
                        </x-secondary-button>
                    </x-confirms-password>
                ...
                @else
                    <x-confirms-password wire:then="showRecoveryCodes">
                        <x-secondary-button class="me-3">
                            {{ __('Afficher Codes de récupération') }}
                        </x-secondary-button>
                    </x-confirms-password>
                @endif
